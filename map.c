...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/04 15:39:41 by md-aless          #+#    #+#             */
/*   Updated: 2022/04/07 14:40:20 by md-aless         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_map2(t_data *v, int i, int j)
{
	...
	if (v->matrix[i][j] == 'M')
		mlx_put_image_to_window(v->mlx, v->win, v->img[3],
			j * 64, i * 64);
	if (v->matrix[i][j] == 'P')
		mlx_put_image_to_window(v->mlx, v->win, v->img[14],
			j * 64, i * 64);
	if (v->matrix[i][j] == 'E')
...
void	put_map3(t_data *v)
{
	...
	if (v->trigger_start == 0)
		...
		mlx_put_image_to_window(v->mlx, v->win, v->img[18],
